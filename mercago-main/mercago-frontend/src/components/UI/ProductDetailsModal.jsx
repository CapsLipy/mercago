    ...
    >
      ...
      >
        ...
        <div style={{ 
          ...
          alignItems: 'center', 
          padding: window.innerWidth <= 768 ? '16px 20px' : '20px 28px', 
          borderBottom: '1px solid #e2e8f0', 
          background: '#f8fafc' 
        }}>
          <h2 style={{ margin: 0, fontSize: window.innerWidth <= 768 ? '1.1rem' : '1.4rem', color: '#0f172a', fontWeight: 700 }}>{product.product_name}</h2>
          <button onClick={onClose} style={{ background: '#f1f5f9', border: 'none', width: '32px', height: '32px', borderRadius: '50%', fontSize: '1.2rem', cursor: 'pointer', color: '#64748b', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            paddingRight: window.innerWidth <= 768 ? '0' : '28px', 
            marginBottom: '20px' 
          }}>
            {/* Image */}
            {product.image_url ? (
              <img src={product.image_url} alt={product.product_name} style={{ width: '100%', height: window.innerWidth <= 768 ? '200px' : '280px', objectFit: 'cover', borderRadius: '12px', marginBottom: '20px' }} />
            ) : (
              <div style={{ width: '100%', height: window.innerWidth <= 768 ? '200px' : '280px', background: 'linear-gradient(135deg, #f1f5f9, #e2e8f0)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px' }}>
              ...
              <div>
                {product.flash_active ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: window.innerWidth <= 768 ? '1.4rem' : '1.8rem', fontWeight: 'bold', color: '#dc2626' }}>
                      ⚡ ₱{Number(product.flash_price).toFixed(2)}
                    </span>
                    <span style={{ textDecoration: 'line-through', color: '#94a3b8', fontSize: '0.9rem' }}>
                      ₱{Number(product.price).toFixed(2)}
                    </span>
                    <span style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: 'normal' }}>/ {product.unit}</span>
                  </div>
                ) : (
                  <span style={{ fontSize: window.innerWidth <= 768 ? '1.4rem' : '1.8rem', fontWeight: 'bold', color: '#0f172a' }}>
                    ₱{Number(product.price).toFixed(2)} <span style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: 'normal' }}>/ {product.unit}</span>
                  </span>
                )}
              </div>
          ...
          <div style={{ 
            ...
            paddingLeft: window.innerWidth <= 768 ? '0' : '28px',
            borderTop: window.innerWidth <= 768 ? '1px solid #e2e8f0' : 'none',
            paddingTop: window.innerWidth <= 768 ? '24px' : '0'
          }}>
